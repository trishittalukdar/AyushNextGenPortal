...
import {
  Plus, X, Building2, MapPin, Wallet, Briefcase, Loader2, TrendingUp, Link2,
} from 'lucide-react';
...
const categoryColors: Record<string, string> = {
  'Tech/Software': 'bg-indigo-100 text-indigo-700',
  'Healthcare & Ayush': 'bg-teal-100 text-teal-700',
  'Core Engineering': 'bg-orange-100 text-orange-700',
  'Business & Finance': 'bg-rose-100 text-rose-700',
  'Government & Public': 'bg-slate-200 text-slate-700',
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Industry Employer Portal</h2>
          <p className="mt-1 text-sm text-slate-500">Post jobs across all sectors and discover ranked candidates</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h3 className="mb-4 flex items-center gap-2 font-semibold text-slate-800">
            <Briefcase className="h-5 w-5 text-emerald-600" /> Publish Job / Internship
          </h3>
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2.5 focus-within:border-emerald-500 focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-500/20">
              {skills.map((skill) => (
                <span key={skill} className="flex items-center gap-1 rounded-md bg-emerald-100 px-2 py-1 text-xs font-medium text-emerald-700">
                  {skill}
                  <button type="button" onClick={() => removeSkill(skill)} className="text-emerald-500 hover:text-emerald-800"><X className="h-3 w-3" /></button>
                </span>
              ))}
              <input value={skillInput} onChange={(e) => setSkillInput(e.target.value)} onKeyDown={handleSkillKeyDown}
                placeholder={skills.length === 0 ? 'Type a skill and press Enter…' : 'Add more…'}
                className="flex-1 bg-transparent py-1 text-sm text-slate-800 placeholder-slate-400 focus:outline-none" />
            </div>
            <div className="mt-3 space-y-2">
              {showAllSkills ? (
                SKILL_CATEGORIES.map((cat) => (
                  <div key={cat.name}>
                    <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{cat.name}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {cat.skills.filter((s) => !skills.includes(s)).map((s) => (
                        <button key={s} type="button" onClick={() => addSkill(s)} className="rounded-full border border-slate-200 px-2 py-0.5 text-xs text-slate-500 transition-colors hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700">+ {s}</button>
                      ))}
                    </div>
                  </div>
                ))
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <button type="button" onClick={() => setShowAllSkills(true)} className="rounded-full border border-emerald-200 px-2 py-0.5 text-xs font-medium text-emerald-600 transition-colors hover:bg-emerald-50">Show all categories</button>
                </div>
              )}
              {showAllSkills && <button type="button" onClick={() => setShowAllSkills(false)} className="text-xs text-slate-400 hover:text-slate-600">Show fewer</button>}
            </div>
          </div>
          ...
          <div className="mt-5 flex justify-end gap-3">
            <button type="button" onClick={() => setShowForm(false)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50">Cancel</button>
      ...
      <div>
        <h3 className="mb-3 flex items-center gap-2 text-lg font-semibold text-slate-800">
          <Briefcase className="h-5 w-5 text-emerald-600" /> Your Posted Jobs
          <span className="text-sm font-normal text-slate-400">({jobs.filter((j) => j.source === 'employer').length})</span>
        </h3>
        {loading ? (
            ...
            {jobs.filter((j) => j.source === 'employer').map((job) => (
              <div key={job.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${categoryColors[job.category] ?? 'bg-slate-100 text-slate-600'}`}>{job.category}</span>
                </div>
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {job.skills.map((s) => <span key={s} className="rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-600">{s}</span>)}
                </div>
                {job.external_url && <p className="mt-2 flex items-center gap-1 text-xs text-blue-600"><Link2 className="h-3 w-3" /> Application URL set</p>}
              </div>
            ))}
            {jobs.filter((j) => j.source === 'employer').length === 0 && (
              <div className="col-span-full rounded-xl border border-dashed border-slate-300 bg-white py-12 text-center text-sm text-slate-400">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center gap-2 border-b border-slate-100 px-5 py-4">
          ...
          <div>
            <h3 className="font-semibold text-slate-800">Applicant Ranking Table</h3>
            <p className="text-xs text-slate-500">Sorted by Skill Match %, practical hours, and status</p>
          </div>
        </div>
        {loading ? (
          <div className="flex justify-center py-12"><Loader2 className="h-7 w-7 animate-spin text-emerald-600" /></div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {sortedCandidates.map((c, idx) => (
                  <tr key={c.id} className="transition-colors hover:bg-emerald-50/40">
                    <td className="px-5 py-3">
                      <span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${
                        idx === 0 ? 'bg-amber-100 text-amber-700' : idx < 3 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
                      }`}>{idx + 1}</span>
                    </td>
                    <td className="px-5 py-3 font-medium text-slate-800">{c.name}</td>
                    <td className="px-5 py-3 text-slate-500">{c.institution}</td>
                    <td className="px-5 py-3">
                      <span className="font-bold text-emerald-600">{c.asq_score}</span>
                      <span className="text-xs text-slate-400">/100</span>
                    </td>
                    <td className="px-5 py-3 text-slate-600">{c.clinical_hours}h</td>
                    <td className="px-5 py-3">
                      <div className="flex flex-wrap gap-1">
                        {c.skills.slice(0, 3).map((s) => <span key={s} className="rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-600">{s}</span>)}
                      </div>
                    </td>
                    <td className="px-5 py-3">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                        c.status === 'Interview Scheduled' ? 'bg-blue-100 text-blue-700' :
                        c.status === 'Shortlisted' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                      }`}>{c.status}</span>
                    </td>
                  </tr>
                ))}
...
const inputClass = 'w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 transition-all focus:border-emerald-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20';
